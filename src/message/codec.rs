...
pub mod rpc {
    ...
    /// Protocol identifier for the economic comparison request-response channel.
    #[derive(Debug, Clone)]
    pub struct CompareProtocol;

    impl AsRef<str> for CompareProtocol {
        fn as_ref(&self) -> &str {
            "/lattice/econ-compare/v1"
        }
    }

    /// CBOR + length-prefix codec for ComparisonRequest/ComparisonResponse.
    #[derive(Debug, Clone, Default)]
    pub struct CompareCodec;

    #[async_trait]
    impl request_response::Codec for CompareCodec {
        type Protocol = CompareProtocol;
        type Request = ComparisonRequest;
        type Response = ComparisonResponse;
